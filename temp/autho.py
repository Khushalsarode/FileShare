...
# Function to get the number of files shared today
def get_files_shared_today():
    today = datetime.combine(date.today(), datetime.min.time())  # Convert date to datetime
    end_of_day = today + timedelta(days=1)  # Calculate the end of the day
    return collection.count_documents({"upload_date": {"$gte": today, "$lt": end_of_day}})
...
files_shared_today = get_files_shared_today()
current_files = get_current_files_count()

# Small panels to display the information
with col1:
    st.info(f"Total Shared Files\n{total_files}")

with col2:
    st.info(f"Files Shared Today\n{files_shared_today}")
...
st.markdown(" 🔄 **Progress bar during file download**")
